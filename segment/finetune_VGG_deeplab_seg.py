...
#from kid_reader_dis4 import ImageReader
#from kid_reader_with_split import ImageReader
...
class DeepLabSEGModel(object):
    ...
    def _restore_weights_and_biases_for_pretrain(self,name):
        ...
        stack_index = int(name[8])
        layer_index = int(name[18])
    ...
    def _get_weights_and_biases(self,name,shape=None,trainable=True):
        if name.startswith("segstack"): # the weights for baseline network
            stack_index = int(name[8]) # note: starts with 0
            layer_index = int(name[18]) # note: starts with 1
            if stack_index < 5:
                with tf.variable_scope("variable",reuse=tf.AUTO_REUSE) as scope:
                    ...
                    shape_w = w.shape
                    w = tf.get_variable(name="%seg_w"%name,initializer=init_w,shape=shape_w)
                    init_b = tf.constant_initializer(b)
                    shape_b = b.shape
                    b = tf.get_variable(name="%seg_b"%name,initializer=init_b,shape=shape_b)
        ...
        with tf.variable_scope("variable",reuse=tf.AUTO_REUSE) as scope:
            w = tf.get_variable(name="%seg_w" % name,shape=shape,initializer=tf.contrib.layers.xavier_initializer_conv2d(dtype=tf.float32))
            b = tf.get_variable(name="%seg_b" % name,shape=[shape[-1]],initializer=tf.contrib.layers.xavier_initializer_conv2d(dtype=tf.float32))
        self.weights[name] = [w,b]
        self.l2_loss += tf.nn.l2_loss(w)
        return w,b
...
#            current = self.upsampling_layer(current, input_batch)
    ...
    def _create_stack(self,current,stack_list,stack_index,pool_size=None,pool_stride=None,keep_prob=None,trainable=True):
        with tf.name_scope("segstack%d" % stack_index) as scope:
            layer_index = 1
            for layer_kind in stack_list:
                w,b = self._get_weights_and_biases("segstack%d_seglayer%d" % (stack_index,layer_index))
                if layer_kind == 1:
    ...
    def upsampling_layer(self, input_prediction, mask_batch):
        with tf.variable_scope("variable",reuse=tf.AUTO_REUSE) as scope:
#            W_t1 = utils.weight_variable([4, 4, 2, 2], name="W_t1")
#            b_t1 = utils.bias_variable([2], name="b_t1")
#            w = tf.get_variable(name="W_t1",shape=[8, 8, 2, 2],initializer=tf.contrib.layers.xavier_initializer_conv2d(dtype=tf.float32))
#            b = tf.get_variable(name="b_t1",shape=[2],initializer=tf.contrib.layers.xavier_initializer_conv2d(dtype=tf.float32))
            initializer = tf.random_normal_initializer(0,0.001)
            w = tf.get_variable(name="W_t1",shape=[11,11,2,2],initializer=initializer)
            initializer = tf.constant_initializer(0)
            b = tf.get_variable(name="b_t1",shape=[2],initializer=initializer)
            conv_t1 = tf.nn.conv2d_transpose(input_prediction, w, [5,321,321,2], strides=[1,8,8,1], padding="SAME", name="layerup") + b
            conv_t1 = tf.nn.relu(conv_t1,name="layerup")
    ...
    def loss_single(self, pred_batch, label_batch):
        ...
        #gt_0 = tf.to_float(tf.argmax(gt,axis=1))
        #gt_0 = tf.Print(gt_0,[tf.reduce_sum(gt_0)],"gt_0")

        #pred = tf.nn.softmax(pred_batch)
        #pred_shape = pred.get_shape()
        #pred_0 = tf.reshape(tf.slice(pred,[0,1],[pred_shape[0],1]),[-1])
        #pred_0 = (1-pred_0)+(2*pred_0-1)*gt_0
        
        #pred_0 = tf.Print(pred_0,[tf.reduce_sum(pred_0)],"pred_0")
        # Pixel-wise softmax loss.
        loss = tf.nn.softmax_cross_entropy_with_logits(labels=gt, logits=pred_batch)
        #loss = tf.Print(loss,[tf.reduce_sum(loss),tf.shape(pred_0),tf.shape(gt_0)],"loss")
        #loss2 = tf.pow((1-pred_0),float(sys.argv[2]))* loss
        #loss2 = loss
        #loss2 = tf.Print(loss2,[tf.reduce_sum(loss2)],"loss2")
        #reduced_loss = tf.reduce_sum(loss2)
        reduced_loss = tf.reduce_sum(loss)
       # reduced_loss = tf.Print(reduced_loss,[tf.reduce_sum(reduced_loss),tf.reduce_sum(loss2)],"reduced_loss")
...
#         testlabel_batchz=tf.cast(testlabel_batch, tf.int32)
...
def density_estimate(pred,density_mask,label):
    ...
    #return right_density_estimate,all_density_estimate-right_density_estimate
